...
func checkTag(field reflect.StructField, tvs []string) error {
...
func checkUint8(fieldName string, tvs []string) error {
	for _, v := range tvs {
		...
		case Eq, Neq, Gt, Gte, Lt, Lte:
			...
			if tv > math.MaxUint8 || tv < math.MinInt8 {
				return overflowErr(fieldName)
			}
		case Between:
			cvs := strings.Split(cds[1], ",")
			...
			if left > math.MaxUint8 || left < math.MinInt8 {
				return overflowErr(fieldName)
			}
			right, err := strconv.Atoi(cvs[1])
			if err != nil {
			...
			if right > math.MaxUint8 || right < math.MinInt8 {
				return overflowErr(fieldName)
			}
			if left >= right {
				return condValLogicErr(fieldName, cds0)
...
func checkUint16(fieldName string, tvs []string) error {
	for _, v := range tvs {
		...
		case Eq, Neq, Gt, Gte, Lt, Lte:
			...
			if tv > math.MaxUint16 || tv < math.MinInt16 {
				return overflowErr(fieldName)
			}
		case Between:
			cvs := strings.Split(cds[1], ",")
			...
			if left > math.MaxUint16 || left < math.MinInt16 {
				return overflowErr(fieldName)
			}
			right, err := strconv.Atoi(cvs[1])
			if err != nil {
			...
			if right > math.MaxUint16 || right < math.MinInt16 {
				return overflowErr(fieldName)
			}
			if left >= right {
				return condValLogicErr(fieldName, cds0)
...
func checkUint32(fieldName string, tvs []string) error {
	for _, v := range tvs {
		...
		case Eq, Neq, Gt, Gte, Lt, Lte:
			...
			if tv > math.MaxUint32 || tv < math.MinInt32 {
				return overflowErr(fieldName)
			}
		case Between:
			cvs := strings.Split(cds[1], ",")
			...
			if left > math.MaxUint32 || left < math.MinInt32 {
				return overflowErr(fieldName)
			}
			right, err := strconv.Atoi(cvs[1])
			if err != nil {
			...
			if right > math.MaxUint32 || right < math.MinInt32 {
				return overflowErr(fieldName)
			}
			if left >= right {
				return condValLogicErr(fieldName, cds0)
...
func checkUint(fieldName string, tvs []string) error {
	for _, v := range tvs {
		...
		case Eq, Neq, Gt, Gte, Lt, Lte:
			...
			if tv > math.MaxUint || tv < math.MinInt {
				return overflowErr(fieldName)
			}
		case Between:
			cvs := strings.Split(cds[1], ",")
			...
			if left > math.MaxUint || left < math.MinInt {
				return overflowErr(fieldName)
			}
			right, err := strconv.Atoi(cvs[1])
			if err != nil {
			...
			if right > math.MaxUint || right < math.MinInt {
				return overflowErr(fieldName)
			}
			if left >= right {
				return condValLogicErr(fieldName, cds0)
